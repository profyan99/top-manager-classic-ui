<template>
  <div class="game-rating-item" :class="{ bankrupt: company.isBankrupt }">
    ...
    <span> $ {{ company.stats.price }} </span>
    <span> $ {{ convertBigNumber(company.stats.revenue) }} </span>
    <span> $ {{ convertBigNumber(company.stats.netProfit) }} </span>
    <span> $ {{ convertBigNumber(company.stats.accumulatedProfit) }} </span>
    <span> {{ company.stats.marketingPart.toFixed(1) }} % </span>
    <span>
