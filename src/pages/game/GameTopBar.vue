...
import { convertGameState, convertSecondsToMinutes } from '~/utils/game';

export default {
  name: 'GameTopBar',
  computed: {
    ...mapState('game', ['currentSecond', 'gameData', 'currentPlayer']),
    currentState() {
      return convertGameState(this.gameData.state, this.currentPlayer.state);
    },
    currentTime() {
